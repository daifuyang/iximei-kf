...
```
sys_role.dataScope                    ← 角色表里一个数字 1-5
   ↓ (login 后 rbac preHandler 加载)
req.currentUser.dataScope              ← 业务 service 直接读
req.currentUser.roleNames              ← 数组,含 超级管理员 / 普通管理员 / 医院管理员 / 客服管理员

业务 service 在 route handler 里读这两个字段,内联判断 + 加 WHERE。
```

**两个判断维度**:
1. `roleNames.includes('超级管理员')` → 无条件看全部 (安全侧门,与 data_scope 值无关)
2. `dataScope === 1/4/5` → 决定走哪条 WHERE 路径
3. **不读 dataScope === 1 (ALL)** 之外也不死板 — 真实逻辑按 roleNames 组合 dataScope 决定

> 不引入通用 `withDataScope(qb, scope)` 抽象 — CRM 用 owner_user_id,医院账号用 crm_医院管理员 JOIN,系统 user 用 dept_id,语义不一样,硬抽象会变 callback。

## `sys_role.dataScope` 五档

| 值 | 名称 | 含义 | 当前用到的角色 |
| --- | --- | --- | --- |
| 1 | ALL | 看全部,无 WHERE 过滤 | 显式 `超级管理员` (因 code lift) 与 `普通管理员` (虽然 普通管理员 当前没有 crm 权限但角色字段保留) |
| 2 | DEPT | 本部门数据 | 暂未用 |
| 3 | DEPT_AND_CHILDREN | 本部门 + 子部门 | 暂未用 |
| 4 | SELF | 仅本人数据 | `客服管理员`、`普通管理员` |
| 5 | CUSTOM | 模块自定义(业务自己定义语义) | `医院管理员` (需要在 crm_医院管理员 模块接 JOIN) |

表字段已存在:`sys_role.dataScope tinyint NOT NULL DEFAULT 1` (`apps/yishan-api/src/db/schema/tables.ts`)。
默认 1 = 看全部,所以 普通管理员 role 即使没刻意接 SELF 过滤,只要 service 没读 dataScope,他看的就是全部。

## 当前 fork 角色矩阵(已 commit 7dedf14)

| 角色 code | menu 可见 | 按键权限 | data_scope | service 看到的请求数据范围 |
| --- | --- | --- | --- | --- |
| `超级管理员` | 全部菜单 | 所有 crm:*/system:* | 1 | **ALL (lift)** |
| `普通管理员` | 仅 account | 仅 `auth:login` | 4 | 默认 SELF (没 crm 权限,403 在 rbac 层拦截) |
| `医院管理员` | /crm/dispatches + account | `crm:dispatches:list/update/reply` + `system:region:list` | 5 | CUSTOM — `crm_dispatch.hospital_id IN (own hospitals)` |
| `客服管理员` | /crm/customers/dispatches/members + account | 客户 CRUD+dispatch / 派单全 / 会员 CRUD+remark | 4 | SELF — owner_user_id = self |

> "客服"是 `客服管理员` 角色 (fork 沿用历史命名,即商务账号);
> "医院管理 / 医院账号"是 `医院管理员` 角色;
> "管理员"是 `普通管理员` 角色 (本 fork 不配置 CRM 权限)。

## 数据流 (用户 GET /crm/v1/customers 时)

```
1. JWT 解码,得到 currentUser.id + currentUser.roleNames: ['客服管理员']
2. rbac preHandler (apps/yishan-api/src/core/plugins/external/rbac.ts)
   - loadForRoleIds(roleIds) 缓存 30s
   - 返回 { perms, roleNames, effectiveDataScope }
   - 超级管理员 旁路 `__super_admin__` 加入 perms
   - 写 req.currentUser.dataScope = effectiveDataScope (本场景: 4)
   - 也带出 roleNames 让 service 用
3. route handler (apps/yishan-api/src/modules/crm/routes/v1/customers/index.ts)
   const roleNames = (req) => req.currentUser?.roleNames ?? []
   const scope     = (req) => req.currentUser?.dataScope ?? 1
   async (req) => CustomersService.list(req.query, uid(req), roleNames(req), scope(req))
4. service (apps/yishan-api/src/modules/crm/services/customers.service.ts)
   function ownerScopeFor(roleNames, userId, scope) {
     if (roleNames.includes('超级管理员')) return undefined  // ALL
...
6. SQL:
   WHERE deleted_at IS NULL
     AND owner_user_id = 4     ← 仅 客服管理员 触发;超级管理员 不加这一行
```
...
function ownerScopeFor(roleNames: ReadonlyArray<string>, scope: DataScopeCode, userId: number): number | undefined {
  if (roleNames.includes(SUPER_ADMIN_NAME)) return undefined  // ALL
  if (scope === DATA_SCOPE.SELF) return userId                  // SELF
  return undefined                                                // 其它档位按 ALL (除非自行实现)
}

export class CustomersService {
  static async list(q: any, userId: number, roleNames: ReadonlyArray<string>, scope: DataScopeCode) {
    const ownerUserId = ownerScopeFor(roleNames, scope, userId)
    return CustomersRepository.list({ ...q, ownerUserId })
  }

  static async getById(id: number, userId: number, roleNames, scope) {
    const c: any = await CustomersRepository.findById(id)
    if (!c) return null
    if (roleNames.includes(SUPER_ADMIN_NAME)) return c
    // SELF:不是 owner 就当不存在
    if (ownerScopeFor(roleNames, scope, userId) === userId && c.ownerUserId !== userId) return null
    return c
  }
...
### 复杂场景:CUSTOM (医院管理员 派单过滤)

`apps/yishan-api/src/modules/crm/services/dispatches.service.ts` — 派单是核心要按代码分支的:

```ts
async function dispatchFilters(roleNames, userId): Promise<{ hospitalIds?, customerOwnerUserIds? }> {
  if (roleNames.includes('超级管理员')) return {}  // 空 = 不过滤
  if (roleNames.includes('医院管理员')) {
    const ids = await HospitalsRepository.accessibleHospitalIds(userId)
    return { hospitalIds: ids.map(x => x.hospitalId) }
  }
  // 客服、普通管理员:派单 -> 客户 -> owner = self
  return { customerOwnerUserIds: [userId] }
}
```

底层走 `crm_医院管理员.hospital_id IN (own)` / `crm_customer.owner_user_id = self`。

> **不要试图把这抽象成 `withCUSTOM(qb, roleNames)`** — 每个 CUSTOM 的实现都不一样。
...
| `src/db/schema/tables.ts` L296 | `sys_role.dataScope tinyint NOT NULL DEFAULT 1` |
| `src/core/permissions/catalog.ts` (`__super_admin__`) | 超级管理员 旁路 sentinel |

> 这些是已有 — **不要去碰**. 业务侧自己读 `req.currentUser.dataScope` 与 `roleNames`。
...
4. **不要忘记 超级管理员 lift** —— 即便 role.dataScope 配置错误,只要 name === '超级管理员',一切放行
5. **不要漏掉 cache invalidation** —— 改了 `sys_role.dataScope` 后要等 30s 或调 `PermissionService.invalidate()` 才能生效

## 何时接 vs 不接

| 接 dataScope | 不接 (默认 ALL) |
| --- | --- |
| 客服 / 业务操作员应该只看自己的 | 后台 普通管理员 看全部 |
...
前置: DB seed 已 run。kefu1 用户绑定 客服管理员 role,普通管理员 用户绑定 超级管理员 role。
...
# 3. kefu1 试图拿 普通管理员 加的 → 应 null (被 SELF 过滤)
curl -sS localhost:3100/api/crm/v1/customers/<普通管理员加的id> -H "Authorization: Bearer $KEFU" \
  | jq '.data'          # 应 null
...
- [ ] 1. 在 `routes/<resource>/index.ts` 加 `roleNames(req)` + `scope(req)` helper,每个 handler 末尾传 `(req.query, uid(req), roleNames(req), scope(req))`
- [ ] 2. service 接 `(userId, roleNames, scope)` 形参
- [ ] 3. service 内部用 `roleNames.includes('超级管理员')` 短路 ALL
...
| 文件 | 用途 |
| --- | --- |
| `apps/yishan-api/src/db/schema/tables.ts` (L296) | `sys_role.dataScope` 字段定义 |
