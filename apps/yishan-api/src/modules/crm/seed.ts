 ...
 * crm 模块的种子入口。
 *
 * 编排脚本（scripts/onboard-modules.ts）通过 spawn `node seed.js` 触发。
 * 行为与 demo/seed.ts 完全一致：从 config/system-menu.json 写入 sys_menu，
 * 并把按钮权限码绑定到 sys_menu_permission。
 */
...
async function bindAllPermissions(flat: FlatNode[]): Promise<void> {
