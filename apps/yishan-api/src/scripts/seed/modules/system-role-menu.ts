...
export async function bindRoleMenusByDefault(db: SeedDb) {
  ...
  const accountMenuIds = menus
    .filter((m) => m.path === '/account' || m.path.startsWith('/account/center'))
    .map((m) => m.id);
  ...
  // 医院账号: /crm 医院管理 + 本院看板 + 派单 + account
  const hospitalMenuIds = menus
    .filter((m) => m.path === '/crm' || m.path.startsWith('/crm/hospitals') || m.path.startsWith('/crm/hospital-dashboard') || m.path.startsWith('/crm/dispatches') || m.path.startsWith('/account'))
    .map((m) => m.id)
